...
export function Monthly(props) {
  ...
  const DefineData = () => {
    ...
    const dataPoints = statistics.dinamically.monthlyStatsInLastYear?.map(
      (item, index) => {
        return item;
      }
    );
  ...
  return (
    <>
      <Stats>
        ...
        {props?.language?.language.User.userPage.visitors}:{' '}
        {statistics.visitors?.monthly}
      </Stats>
      <Stats>
        <ImCheckmark color="#2bdfd9" />
        {props?.language?.language.User.userPage.followers}:{' '}
        {statistics.followers?.monthly}
      </Stats>
      <Stats>
        <BiStar color="#bb3394" />
        {props?.language?.language.User.userPage.stars}:{' '}
        {statistics.stars?.monthly}
      </Stats>
      <Stats>
